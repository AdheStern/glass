...
describe.skipIf(!ON_EPHEMERAL)("trigger trg_stock_movement_apply", () => {
  ...
  afterAll(async () => {
    ...
    await prisma.product.deleteMany({ where: { slug: { startsWith: "trg-" } } });
    await prisma.$disconnect();
  });

  it("aplica el delta de cada asiento", async () => {
    await prisma.stockMovement.create({
      data: { variantId, kind: "CARGA_INICIAL", qty: 10, occurredAt: new Date(), sourceType: "test" },
    });
    await prisma.stockMovement.create({
      data: { variantId, kind: "VENTA", qty: -3, occurredAt: new Date(), sourceType: "test" },
    });

    const stock = await prisma.variantStock.findUnique({ where: { variantId } });
    expect(stock?.qty).toBe(7);
  });

  it("una salida mayor a la existencia deja el resumen negativo (alerta, no error)", async () => {
    await prisma.stockMovement.create({
      data: { variantId, kind: "VENTA", qty: -100, occurredAt: new Date(), sourceType: "test" },
    });
    const stock = await prisma.variantStock.findUnique({ where: { variantId } });
